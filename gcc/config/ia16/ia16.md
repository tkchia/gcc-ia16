...
(define_insn_and_split "tablejump"
  [(parallel
    [(set (pc) (match_operand:HI 0 "nonimmediate_operand" "rm"))
     (use (label_ref (match_operand 1)))])]
  ""
  "jmp\t{*}%0"
  "TARGET_CS_JUMP_TABLES
   && MEM_ADDR_SPACE (operands[0]) != ADDR_SPACE_SEG_CS"
  [(parallel
    [(set (pc) (match_operand:HI 0 "nonimmediate_operand" "rm"))
     (use (label_ref (match_operand 1)))])]
{
  ...
  gcc_assert (! LABEL_REF_NONLOCAL_P (operands[1]));
  addr = shallow_copy_rtx (addr);
  set_mem_addr_space (addr, ADDR_SPACE_SEG_CS);
  operands[0] = addr;
})
